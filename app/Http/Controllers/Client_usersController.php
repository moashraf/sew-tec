...
class Client_usersController extends Controller
{
    //
    ...
    public function getClientusersdatatable() {

        $users = DB::table('users')->where('type','client')
                ->select('id','username', 'email' , 'phone' ,'active');
        return Datatables::queryBuilder($users)
                        ...
                        ->editColumn('active', '@if($active == "active") مفعل @elseif($active == "deactive") غير مفعل @endif')
                          ->addColumn('action', '<a type="button" data-toggle="modal" data-target="#edit" href="{{url("clientusers/editusers/$id")}}" class="btn green edit">تعديل</a>
                    <button type="button" class="btn red" data-toggle="confirmation" action="Delete(\'{{ url("/clientusers/deleteoneuser/$id") }}\', element);" >حذف</button> ')
                        ->make();
    }
    ...
    public function getEditusers($id) {
        ...
        return view('admin.users.edit.clientusers_edit')->with('user', $user);
    }
